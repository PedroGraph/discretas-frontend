...
export const formatShoppingCartData = (orderData) => {
  const { paymentId, address, city, state, shoppingCart } = orderData;
  
  const order = Object.values(shoppingCart).map(item => ({
      userId: item.userId,
      ...
      images: item.product.images.map(img => img.imageName) 
  }));
...
export const googleFieldsValidation = (googleFields) => {
  const { given_name, family_name, email, picture } = googleFields.additionalUserInfo.profile;
  const { accessToken } = googleFields.user._delegate.stsTokenManager
  return {
    email,
    password: uuidv4().split('').map((char, index) => (index % 2 === 0 ? char.toUpperCase() : char.toLowerCase())).join(''),
...
export const objectToDatabaseQuery = (obj) => {
  return Object.entries(obj)
      .map(([key, value]) => `${encodeURIComponent("product"+key)}=${encodeURIComponent(value)}`)
      .join("&");
}