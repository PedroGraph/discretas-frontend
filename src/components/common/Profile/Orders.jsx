      ...
      <div className="flex flex-col gap-4 pt-4 px-72 bg-gray-100 items-center">
        <div className="grid grid-cols-3 w-3/4">
          <a href="/orders" className={`border-b-2 text-center ${selectedSection.includes("orders") ? "border-black" : "border-gray-300"} cursor-pointer no-underline`}>
            <span className={`text-sm font-bold text-center`}>Pedidos Creados</span>
          </a>
          <a href="/orders/recieved" className={`border-b-2 text-center ${selectedSection.includes("/recieved") ? "border-black" : "border-gray-300"} cursor-pointer no-underline`}>
            <span className={`text-sm font-bold text-center`}>Pedidos Recibidos</span>
          </a>
          <a href="/orders/canceled" className={`border-b-2 text-center ${selectedSection.includes("/canceled") ? "border-black" : "border-gray-300"} cursor-pointer no-underline`}>
            <span className={`text-sm font-bold text-center`}>Pedidos Cancelados</span>
          </a>
        </div>
        {orders.length > 0 && (
            ...
            return (
              <div className="border-1 border-gray-300 rounded w-full" key={index}>
                <div className="grid grid-cols-3 bg-gray-200 gap-2 py-3 px-4">
                  ...
                  <span className="col-span-3">Enviado a: <span className="font-semibold text-sm">{order.shippingAddress.address} {order.shippingAddress.city}</span></span>
                </div>
                <div className="grid grid-cols-1 ">
                    ...
                    })}
                  {
                    !Array.isArray(order.products) && order.products && (
                      <div className="flex gap-4 py-2 border-1 border-gray-200 px-4 py-2" >
                        <Link className="max-w-[190px]" onClick={() => { window.location.href = `/lubricante/${order.products.id}` }}>
                          <img src={order.products.images[0].imageName} alt="" className="rounded" />
                        </Link>
                        <div className="flex flex-col max-w-[60%]">
                          <Link onClick={() => { window.location.href = `/lubricante/${product.id}` }}>
                            <span className="font-semibold text-sm">{order.products.name}</span>
                          </Link>
                          <span className="pt-1">
